...
object JulyInstructionMediator {
    ...
    private fun dispatch(instructionData: JulyInkInstruction, questInstance: QuestInstance) {
        if (!executors.containsKey(instructionData.name)) {
            logger.warning("Illegal instruction ${instructionData.name}! (Caused by player ${questInstance.player.name}, line ${questInstance.story.currentText}))")
    ...
    fun dispatch(instructionsData: List<JulyInkInstruction>, questInstance: QuestInstance) {
        for (instructionData: JulyInkInstruction in instructionsData) {
            dispatch(instructionData, questInstance)
        }
    }
